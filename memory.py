...
blank_trans = Transition(0, torch.zeros(3, dtype=torch.uint8), None, 0, False)
...
class ReplayMemory():
  ...
  def sample(self, batch_size):
    ...
    # TODO: ugly solution (flattening returns)
    actions, returns, nonterminals = torch.cat(actions), torch.cat([x.view(-1) for x in returns]), torch.stack(nonterminals)
